...
#[test]
fn macos_resolves_found_when_the_containing_directory_exists() -> Result<(), Box<dyn std::error::Error>> {
    let home = tempdir()?;
...
#[test]
fn macos_resolves_not_found_when_no_claude_directory_exists() -> Result<(), Box<dyn std::error::Error>> {
    let home = tempdir()?;
...
#[test]
fn windows_resolves_found_for_exactly_one_matching_package_folder() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        resolution,
        HostPathResolution::Found(package.join("LocalCache/Roaming/Claude/claude_desktop_config.json"))
    );
    Ok(())
}

#[test]
fn windows_resolves_ambiguous_for_two_matching_package_folders() -> Result<(), Box<dyn std::error::Error>> {
    let packages = tempdir()?;
    for suffix in ["Claude_abc123", "Claude_def456"] {
        std::fs::create_dir_all(packages.path().join(suffix).join("LocalCache/Roaming/Claude"))?;
    }
...
#[test]
fn windows_rejects_a_stale_package_folder_missing_the_real_subpath() -> Result<(), Box<dyn std::error::Error>> {
    let packages = tempdir()?;
    // Name prefix matches, but the real `LocalCache/Roaming/Claude` subpath
    // is absent: a leftover folder from an old install or a reinstall must
    // not be mistaken for a real, current one.
...
#[test]
fn windows_ignores_folders_that_do_not_match_the_name_prefix() -> Result<(), Box<dyn std::error::Error>> {
    let packages = tempdir()?;
...
#[test]
fn windows_resolves_not_found_when_the_packages_directory_itself_is_missing() -> Result<(), Box<dyn std::error::Error>>
{
    let packages = tempdir()?;
...
#[test]
fn windows_roaming_resolves_found_when_the_containing_directory_exists() -> Result<(), Box<dyn std::error::Error>> {
    let roaming = tempdir()?;
...
#[test]
fn windows_roaming_resolves_not_found_when_no_claude_directory_exists() -> Result<(), Box<dyn std::error::Error>> {
    let roaming = tempdir()?;
...
#[test]
fn windows_combined_finds_an_msix_install_with_no_roaming_install_present() -> Result<(), Box<dyn std::error::Error>> {
    let packages = tempdir()?;
    let package = packages.path().join("Claude_abc123xyz");
    std::fs::create_dir_all(package.join("LocalCache/Roaming/Claude"))?;
    let roaming = tempdir()?;
...
#[test]
fn windows_combined_reports_ambiguous_when_both_layouts_have_a_candidate() -> Result<(), Box<dyn std::error::Error>> {
    let packages = tempdir()?;
    let package = packages.path().join("Claude_abc123xyz");
    std::fs::create_dir_all(package.join("LocalCache/Roaming/Claude"))?;
    let roaming = tempdir()?;
