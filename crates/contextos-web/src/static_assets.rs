//! `GET /static/{{path}}`: non-vault server assets only, never
//! vault content.
...
/// path-escape check in this workspace: don't re-implement a
/// security-relevant check when a well-tested one already exists at the
