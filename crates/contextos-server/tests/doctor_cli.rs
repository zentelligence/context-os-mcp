...
#[test]
fn phase_2_doctor_validates_configuration_and_reports_a_healthy_vault()
...
#[test]
fn stage_4_doctor_reports_semantic_search_disabled_by_default()
-> Result<(), Box<dyn std::error::Error>> {
    let fixture = tempdir()?;
    let data_home = tempdir()?;
    let vault = fixture.path().join("vault");
...
#[test]
fn phase_2_doctor_returns_failure_with_an_action_for_missing_indexes()
-> Result<(), Box<dyn std::error::Error>> {
    let fixture = tempdir()?;
    let data_home = tempdir()?;
    let vault = fixture.path().join("vault");
...
#[test]
fn fr_94_doctor_resolve_flag_rebuilds_a_stale_index_and_reports_it_healthy()
-> Result<(), Box<dyn std::error::Error>> {
    let fixture = tempdir()?;
    let data_home = tempdir()?;
    let vault = fixture.path().join("vault");
...
#[test]
fn fr_94_doctor_resolve_dry_run_previews_without_writing() -> Result<(), Box<dyn std::error::Error>>
{
    let fixture = tempdir()?;
    let data_home = tempdir()?;
    let vault = fixture.path().join("vault");
