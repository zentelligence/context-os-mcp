...
#[test]
fn derives_title_headings_tags_and_body_from_note() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(document.tags(), ["project/alpha", "review", "status/active"]);
    assert!(document.body().contains("# Alpha Overview"));
    assert!(!document.body().contains("title: Alpha Note"));
    assert_eq!(
        document.frontmatter().get("title").and_then(|value| value.as_str()),
        Some("Alpha Note")
    );
...
#[test]
fn title_falls_back_to_first_heading_then_filename() -> Result<(), Box<dyn std::error::Error>> {
    let vault = tempfile::tempdir()?;
    let modified = timestamp()?;
...
#[test]
fn invalid_frontmatter_degrades_to_body_only_document() -> Result<(), Box<dyn std::error::Error>> {
    let vault = tempfile::tempdir()?;
    let content = "---\ntitle: [unclosed\n---\n\n# Recovered Heading\n\nProse.\n";
...
#[test]
fn inline_tags_respect_code_and_numeric_rules() -> Result<(), Box<dyn std::error::Error>> {
    let vault = tempfile::tempdir()?;
